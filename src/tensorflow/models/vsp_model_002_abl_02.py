## from vsp_model_002.py: simple alignment
        ...
        with tf.variable_scope('objective'):
            with tf.variable_scope('triplet_mil'):
                ...
                def cost_fn(inputs):
                    ...
                    return (ent_loss_1, pred_loss_1, role_loss_1, ent_conf_loss_1, pred_conf_loss_1,
                            conf_accuracy_1, pred_conf_accuracy_1, box_loss_1, iou_1, 
                           ) 
                (
                    ent_loss_1, pred_loss_1, role_loss_1, ent_conf_loss_1, pred_conf_loss_1,
                    conf_accuracy_1, pred_conf_accuracy_1, box_loss_1, iou_1,
                ) = tf.map_fn(cost_fn, [
                ...
                ent_loss = tf.reduce_mean(ent_loss_1, axis=0)
                ent_conf_loss = tf.reduce_mean(ent_conf_loss_1, axis=0)
                pred_loss = tf.reduce_mean(pred_loss_1, axis=0)
                pred_conf_loss = tf.reduce_mean(pred_conf_loss_1, axis=0)
                role_loss = tf.reduce_mean(role_loss_1, axis=0)
                        
                loss_generator = {
                    'loss_factor_g_mil_ent': ent_loss,
                    'loss_factor_g_mil_ent_conf': ent_conf_loss,
                    'loss_factor_g_mil_pred': pred_loss,
                    'loss_factor_g_mil_pred_conf': pred_conf_loss,
                    'loss_factor_g_mil_role': role_loss,
                }
                effective_loss = tf.add_n([self.get_hyperparam(key) * val for key, val in loss_generator.items()])

                self.add_to_log(conf_accuracy_1, f'conf_accuracy_entfirst')
                self.add_to_log(pred_conf_accuracy_1, f'pred_conf_accuracy_entfirst')
                self.add_to_log(box_loss_1, f'box_loss_entfirst')
                self.add_to_log(iou_1, f'iou_entfirst')
            ...
            for k, v in loss_generator.items():
                tf.summary.scalar(f'{k}_entfirst', v)
